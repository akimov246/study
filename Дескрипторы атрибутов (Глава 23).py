...
'''
Функция внутри класса становится связанным методом, потому что у всех определенных пользователем функций имеется 
метод __get__, а значит, будучи присоединенныим к классу, они ведут себя как дескрипторы.
'''
obj = Managed()
print(obj.spam) # Чтение obj.spam возвращает объект, представляющий связанный метод
print(Managed.spam) # Однако чтение Managed.spam возвращает функцию
obj.spam = 7
print(obj.spam) # Присваивание атрибуту obj.spam маскирует атрибут класса, делая матод spam недоступным через объект obj
...
word = Text('forward') # Представление repr экземпляра Text выглядит как вызов конструктора Text, создающего точно
                       # такой же экземпляр
print(word)
print(word.reverse()) # Метод reverse возвращает инвертированный текст
print(Text.reverse(Text('backward'))) # Метод, вызванный от имени класса, работает как функция.
print(type(Text.reverse), type(word.reverse)) # (<class 'function'>, <class 'method'>)
print(list(map(Text.reverse, ['repaid', (10, 20, 30), Text('stressed')]))) # Метод Text.reverse работает как функция и
                                                                           # применим даже к объектам, не являющимся
                                                                           # экземплярами Text
print(Text.reverse.__get__(word)) # Любая функция является непереопределяющим дескриптором. Если вызвать ее метод __get__
                                  # от имени экземпляра, то будет возвращен метод, связанный с этим экземпляром
print(Text.reverse.__get__(None, Text)) # Если вызвать метод __get__, указав в качестве аргумента instance объект None, то
                                        # будет возвращена сама функция
print(word.reverse) # Выражение word.reverse приводит к вызову Text.reverse.__get__(word) и возврату связанного метода
print(word.reverse.__self__) # У объекта связанного метода имеется атрибут __self__, в котором хранится ссылка на экземпляр,
                             # от имени которого вызывается метод
print(word.reverse.__func__ is Text.reverse) # В атрибуте __func__ связанного метода хранится ссылка на исходную функцию,
                                             # присоединенную к управляемому классу.

# Советы по искользованию дескрипторов
'''
> Для простоты пользуйтесь классом property
Встроенный класс property создает переопределяющие дескрипторы, в которых реализованы оба метода __set__ и __get__,
даже если вы сами не задавали метод установки (метод __delete__ также предоставляется декоратором property, даже если вы
сами не определяли никакого метода удаления). Подразумеваемый по умолчанию метод __set__ возбуждает исключение
AttributeError: can't set attribute, поэтому свойство - это простеший способ создать доступный только для чтения атрибут
и избежеть проблемы, описанной ниже.

> В дескрипторах только для чтения необходим метод __set__
Если вы используете дескрипторный класс для реализации атрибута, допускающего только чтение, то не забывайте реализовать оба
метода __get__ и __set__, иначе одноименный атрибут экземпляра замаскирует дескриптор. Метод __set__ атрибута, доступного
только для чтения, должен просто возбуждать исключение AttributeError с подходящим сообщением.

> Проверяющим дескрипторам достаточно одного метода __set__
Если дескриптор предназначен только для проверки значений, то метод __set__ должен проверять полученный аргумент value и,
если он правилен, устанавливать значение непосредственно в атрибуте __dict__ экземпляра, используя в качестве ключа 
имя экземпляра дескриптора. Тогда чтение атрибута с таким же именем из экземпляра будет производиться максимально быстро, т.к.
не требует наличия метода __get__.

> Кеширование можно эффективно реализовать при наличии одного лишь __get__
Если вы напишите метод __get__, то получите непереопределяющий дескриптор. Они полезны, когда требуется выполнить накладные
вычисления и кешировать результат, установив атрибут экземпляра с таким же именем. Одноименный атрибут экземпляра
маскирует дескриптор, поэтому при последующем доступе к этому атрибуту значение будет извлекаться непосредственно из атрибута
__dict__ экземпляра в обход метода __get__ дескриптора. Декоратор @functools.cached_property на самом деле порождает 
непереопределяющий дескриптор.

> Неспециальные методы можно замаскировать атрибутами экземпляра
Поскольку в функциях и методах реализован только метод __get__, они не перехыватывают попытки установить одноименные атрибуты
экземпляра, так что после простого присваивания my_obj.the_method = 7 последующий доступ к the_method через данный экземпляр
вернет число 7, хотя на других экземплярах это никак не отразится. Однако на специальные методы это не распространяется.
Интерпретатор ищет специальные методы только в самом классе, т.е. repr(x) всегда вычисляется как x.__class__.__repr__(x), 
так что наличие атрибута __repr__ в x не влияет на результат repr(x). По той же причине существование атрибута с именем
__getattr__ в экземпляре не испортит обычный алгоритм доступа к атрибутам. 
'''